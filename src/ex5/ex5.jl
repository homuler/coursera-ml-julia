...
# Plot training data
scatter(X, y, marker="x", s=20, linewidth=1.5, color="r")
...
scatter(X, y, marker="x", s=10, linewidth=1.5, color="r")
xlabel("Change in water level (x)")
ylabel("Water flowing out of the dam (y)")
hold(true)
plot(X, [ones(m, 1) X]*theta, linestyle="--", linewidth=2)
hold(false)
...
figure(1)
scatter(X, y, marker="x", s=10, linewidth=1.5, color="r")
plotFit(minimum(X), maximum(X), mu, sigma, theta, p)
xlabel("Change in water level (x)")
ylabel("Water flowing out of the dam (y)")
